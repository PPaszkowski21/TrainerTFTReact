...
import RoundTimer from '../choosemode/roundTimer';
import RoundManagement from '../choosemode/roundManagement';
import { useTimer } from 'use-timer';
import { LEVEL_EXP } from '../settings/constants';

export default function RefreshTrainingRS({displayRefreshTraining, board, setBoard, shopOffer, setShopOffer, copiesOffTheDeck, championObjective, rerollCounter, setRerollCounter, timeReroll, roundSimulator}) {
    ...
    function getMaxExp() {
        let value = LEVEL_EXP[level];
        setMaxExp(value);
    }

    function addExp(value,isPaid) {
        if(level===9){
            return;
        }
        if(isPaid){
    ...
    return (
        ...
        <div className="refreshTrainingContainer">
            <div className="traitsAndReroll">
                ...
                <div className="timerAndRoundManagement">
                    <RoundTimer time={time} start={start} pause={pause} reset={reset} status={status} goNextRound={goNextRound} ></RoundTimer>
                    <RoundManagement setGold={setGold} gold={gold} roundNumber={roundNumber}> </RoundManagement>
                </div>
                <RerollCounter pause={pause} reset={reset} time={time} status={status} roundSimulator={roundSimulator} level={level} board={board} rerollCounter={rerollCounter} setRerollCounter={setRerollCounter} championObjective={championObjective} copiesOffTheDeck={copiesOffTheDeck}/>
            </div>
            ...
            <ControlPanel maxExp={maxExp} exp={exp} setExp={addExp} timeReroll={timeReroll} roundSimulator={roundSimulator} board={board} setBoard={addToBoard} level={level} setLevel={lvlChange} gold={gold} setGold={setGold} shopOffer={shopOffer} setShopOffer={setShopOffer} copiesOffTheDeck={copiesOffTheDeck} rerollCounter={rerollCounter} setRerollCounter={setRerollCounter}/> 
        </div> : null
    );
}